...
> [!WARNING] Possible Conflicts
> ${conflictLines}

> [!IMPORTANT] Sensitive Artifacts
> ${sensitive.length
        ? sensitive.map(s => `- ${s.status} ${s.file} (pattern: ${s.pattern}, reason: ${s.reason || "sensitive artifact"})`).join("\n> ")
        : "- None detected"}
