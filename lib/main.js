...
async function upsertComment(prNumber, body) {
  ...
  const marker = "<!-- pr-intelligence-report -->";
...
(async () => {
  ...
  const conflictLines = overlaps.length
    ...
    : `- None detected among the sampled open PRs`;
...
### Developer State
🧠 Focused  
${state.fatigue ? `⚠️ Fatigue detected (**${state.lateWeek}** late-night commits in last 7 days)` : `✅ No fatigue signal (late-night commits last 7d: **${state.lateWeek}**)`}
...
- Contributors touching these files (last 180d): ${mentions.map(c => `\`${c}\``).join(", ")}
- Bus factor: **${own.busFactor}${own.busFactor === 1 ? " ⚠️" : ""}** — how many people can vanish before this area becomes hard to maintain

### Refactor Safety Score
**${safety.score}%**  
- +${additions} / -${deletions} lines  
- Tests touched: **${safety.testsTouched ? "Yes" : "No"}**  
- Public API surface touched: **${safety.publicTouched ? "Yes" : "No"}**
`.trim();

  await upsertComment(prNumber, md);
  console.log("Posted/updated PR intelligence comment");
})().catch((e) => {
  console.error(e);
  process.exit(1);
});
